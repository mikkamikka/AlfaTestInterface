...
{
    public class IconBoxAudio : IconBox
    {
        private string audioclipName { get; set; }
        Matrix view;
        Matrix projection;
        private AudioWindow audio_sample_window;

        public IconBoxAudio(Game game, string _audioclipName)
            : base(game)
        {
        ...
        {
            LoadContent(Game.Content, "icon_audiobox");

            audio_sample_window = new AudioWindow(Game);
            audio_sample_window.LoadContent(Game.Content, "audioplayer_pane", audioclipName);
           // audio_sample_window.spriteBatch = new SpriteBatch(GraphicsDevice);
        ...
        {
            audio_sample_window.Update(gameTime);
            
            

            if (audio_sample_window.audioWindowIsOpen) current_hilite = new Vector3(0.0f, 0.2f, 0.0f);
            else current_hilite = new Vector3(-0.1f, -0.1f, -0.1f);
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            if (audio_sample_window.audioWindowIsOpen)
            {
                audio_sample_window.Draw();                         //рисуем окно с медиа
            }
            else
            {
            ...
            if (!audio_sample_window.audioWindowIsOpen & audio_sample_window.previousAudioWindowIsOpen)         // если состояние медиа окна меняется с открытого на закрытое
            {
                    ...
                    case ContainmentType.Intersects:
                        ...
                        if (AvateeringXNA.cursorVectorDirection.Z > 0.3f)
                        {
                            if ((previousCollideResult1 == ContainmentType.Disjoint) &&
                                (audio_sample_window.audioInstance.State == SoundState.Playing))
                            {
                                //GestureAnyIsActive = true; timer.Start(1000f);
                                //audio_sample_window.BringToBack();
                                     ...
                                     (audio_sample_window.audioInstance.State != SoundState.Playing))
                            {
                                ...
                                audio_sample_window.BringToFront();
                                audio_sample_window.StartPlaying();
                                //PressButtonEffectActivate();
                                HideIconBox();
                            }
                        }
                        break;
                    default:
                        // Window.Title = "___________________";
                        break;
                }
